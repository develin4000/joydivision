...
report_t buildReport(uchar joyport)
{
   ...
   if (joyport == JOYPORT1)
   {
      ...
      if (tmp & (1<<PC1)) {b2 = 1;} // Button 2

      currState = PINB;
      tmp = currState^0xff;

      if (tmp & (1<<PB1)) {b1 = 1;} // Button 1
   }
   else // joyport == JOYPORT2
   {
      ...
      if (tmp & (1<<PB2)) {b2 = 1;} // Button 2
      if (tmp & (1<<PB3)) {b1 = 1;} // Button 1
...
static void hardwareInit(void)
{
   ...
   Pin   Joystick (Amstrad)    AVR-Pin (Port 1)   AVR-Pin (Port 2)
   1     Up                    PC5                PD7
   2     Down                  PC4                PD6
   3     Left                  PC3                PD5
   4     Right                 PC2                PD4
