...
report_t buildReport(uchar joyport)
{
   ...
   {  
      ...
      for (cntr=0; cntr<16 ; cntr++)  // We need to shift through all buttons to check their state...
      {
         ...
         switch (cntr)
         {
            case 0  : if (!TESTBIT(PINC, PC3)) buttons |= 0x01; break; // B
            case 1  : if (!TESTBIT(PINC, PC3)) buttons |= 0x02; break; // Y
            ...
            case 8  : if (!TESTBIT(PINC, PC3)) buttons |= 0x10; break; // A
            case 9  : if (!TESTBIT(PINC, PC3)) buttons |= 0x20; break; // X
            case 10 : if (!TESTBIT(PINC, PC3)) buttons |= 0x40; break; // L
            case 11 : if (!TESTBIT(PINC, PC3)) buttons |= 0x80; break; // R
...
static void hardwareInit(void)
{
   ...
   Pin   SNES-adapter   AVR-Pin (Port 1)   AVR-Pin (Port 2)
   1                    PC5                PD7
   2                    PC4                PD6
   3     Data Out       PC3                PD5
   4     Latch          PC2                PD4
